...
def download_kafka(target):
    r = requests.get(
        "http://apache.mirror.globo.tech/kafka/1.0.0/" "%s.tgz" % KAFKA_FILE,
        stream=True,
    )
    ...
    subprocess.check_call(
        ["tar", "xzf", KAFKA_FILE], cwd=os.path.dirname(target)
    )
...
def launch_kafka():
    ...
    cmd = (
        "docker run -d -p 2181:2181 -p 9092:9092 --env "
    ...
    def predicate():
        try:
            out = subprocess.check_output(
                ["docker", "logs", cid], stderr=subprocess.STDOUT
            )
...
def create_kafka_topic(cid, topic, partitions):
    cmd = (
        "docker exec {cid} /opt/kafka_2.11-0.10.1.0/bin/kafka-topics.sh "
...
def test_from_kafka():
    ...
    with kafka_service() as kafka:
        ...
        wait_for(
            lambda: len([o for o in out if o]) == startlen + 10, 10, period=0.1
        )  # list may include some test data
...
def test_kafka_batch():
    ...
    with kafka_service() as kafka:
        ...
        wait_for(
            lambda: any(out) and out[-1][-1] == b"value-9", 10, period=0.2
        )
        stream.stopped = True
